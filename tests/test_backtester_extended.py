...
def test_backtester_calculates_sharpe_and_drawdown():
    ...
    # Assert drawdown calculations (prices went straight up, so peak drawdown should be minimal/0)
...
def test_backtester_calculates_win_rate():
    ...
    # Index 2: 8 -> SMA(2) = 10. Close < SMA. SELL signal (profitable or lossy depending on entry/exit).
    ...
    result = run_backtest(price_data, strategy_type="sma", window=2, transaction_fee_rate=0.0)
    # Trades triggered:
